...
//				exvalr=Math.min(exvalr, lcar);
	...
	public int intervalSearch_resTrivial(String[] bound, double l, double r,
			int sign) {
		double[] res = new double[maxh * 2];
		int resnum = intervalSearch(l, r, res);
		
        double st=100000000000.0, ed=0.0;
		...
		// tmp = (long) res[i];
	    bound[0] = prekey_cons((long) (st)) + "," + "0";
		bound[1] = prekey_cons((long) (ed) + 1) + "," + "0";
...
//		midr = Math.min(lcar, rb);
	...
	public int pointSearch_resMr(String[] bdrow, double val) {
		double[] res = new double[maxh];
		int resnum = pointSearch(val, res);
		String prekey = "";
		
		double lbd=0.0, rbd=0.0;
		for (int i = 0; i < resnum; ++i) {

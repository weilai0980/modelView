...
public class QueryManager {
	...
	public void newtiExp(int num) throws Exception {
		...
		textRecordExp_clear("./result/newTiExp.txt");
		...
		for (int i = 0; i < num; ++i) {
			...
			nkviTemp.kvi_rangeSearchEnhan(inte[0], inte[1], qual, 0);
...
//			System.out.printf("%f  \n", nkviTemp.kvi_timeCnt());
			...
			nkviTemp.kvi_rangeSearchIndiviPerReg(inte[0], inte[1], qual, 0);
			nkviTemp.kvi_metricOutput(tmpstat, tuprange);
			stat[tnum][0] = tmpstat[0] - 1.0; // tdur
			stat[tnum][1] = tmpstat[1];// inmem
			stat[tnum][2] = tmpstat[2];// randacc
			...
			textRecordExp("./result/newTiExp.txt", stat, tuprange, tnum,
					inte[0], inte[1]);
		}
	...
	public void newtiviExp(int num) throws Exception {
		...
		for (int i = 0; i < num; ++i) {
			...
			qryGen_tempRange(intet);
			qryGen_valRange(intev);
			...
			nkviTemp.kvi_rangeSearchScanPerReg(intet[0], intet[1], qual, 0);
			nkviTemp.kvi_metricOutput(tmpstat, tuprange);
			stat[tnum][0] = tmpstat[0] - 1.0; // tdur
			stat[tnum][1] = tmpstat[1];// inmem
			stat[tnum][2] = tmpstat[2];// randacc

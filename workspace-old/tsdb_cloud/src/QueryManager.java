...
public class QueryManager {
	...
	public QueryManager(boolean format) throws IOException {
		...
		if (format == true) {
			...
			kviTemp = new kviManager("tempIdxmodCol", true);

			// // test
			// String str = kviTemp.idxtabname;

			kviVal = new kviManager("valIdxmodCol", true);
			// test
			// String str1 = kviTemp.idxtabname;

		} else {
			kviTemp = new kviManager("tempIdxmodCol", false);

			// // test
			// String str = kviTemp.idxtabname;

			kviVal = new kviManager("valIdxmodCol", false);
	...
	public void ImportFolder(String foldername, String filter, int model)
		...
		if (model == 0) {
			// onlinModIni();
			// fhb.delTab("tempRaw_guo");
			// fhb.creTab("tempRaw_guo");
			fhb.iniTabOperation("tempRaw_guo");

			// fhb2.delTab("tempRaw_val");
			// fhb2.creTab("tempRaw_val");
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (isnull == 1) {
					...
					if (fhb.put("tempRaw_guo",
							rawid[0] + ":" + rawdata[rds][0], "attri", "val",
							rawdata[rds][tsno]) == 0)
						System.out.println("raw data failure");

					if (fhb2.put("tempRaw_val", rawid[0] + ":"
							+ valReg(rawdata[rds][tsno]) + ":"
							+ rawdata[rds][0], "attri", "temp", rawdata[rds][0]) == 0)
						System.out.println("raw data failure");
				}
				// .......................................................//
				rds = 1 - rds;
	...
	public void tiExp(int num) throws Exception {
		...
		textRecordExp_clear("./result/tiExp.txt");
		...
		for (int i = 0; i < num; ++i) {
			...
			kviTemp.kvi_rangeSearchCol(inte[0], inte[1], qual);
			...
			// ....for cluster..//
			// kviTemp.conve_kvi_rangeSearchConsec(inte[0], inte[1]);
			...
			//
			startCnt();
//			stat[tnum][1] = filterHb.rangeQuery("tempM1_guo", inte[0], inte[1],
//					"attri", qual);
			stat[tnum++][0] = stopCnt();
...
//					"tempRaw_guo");
			stat[tnum++][0] = stopCnt();
			//
			stat[tnum][0] = kviMRpro.numMapIn;
			stat[tnum++][1] = qualnum;
			//
			// // // for no-gridding process
			//
//			kviTemp.kvi_rangeSearchNoGrid(inte[0], inte[1]);
//			kviTemp.kvi_metricOutput(tmpstat, tuprange);
			stat[tnum++][0] = tmpstat[0]; // tdur
			// stat[tnum][1] = tmpstat[1];// inmem
			// stat[tnum][2] = tmpstat[2];// randacc
			...
			textRecordExp("./result/tiExp.txt", stat, tuprange, tnum, inte[0],
					inte[1]);
		}
	...
	public void tpExp(int num) throws Exception {
		...
		textRecordExp_clear("./result/tpExp.txt");
		fhb.creTab("tempM1_guoRes");
		...
		for (int i = 0; i < num; ++i) {
			...
			startCnt();
			qualnum = mrscanner.jobTempPoint(inte[0], "tempM1_guo");
			stat[tnum][0] = stopCnt();
			stat[tnum++][1] = 100;

			startCnt();
			stat[tnum][1] = filterHb.pointQuery("tempM1_guo", inte[0], "attri",
					qual);
			stat[tnum++][0] = stopCnt();
			...
			stat[tnum][0] = kviMRpro.numMapIn;// from kvi
			stat[tnum++][1] = qualnum;// from pure mapreduce

			textRecordExp("./result/tpExp.txt", stat, tuprange, tnum, inte[0]);
		}
	...
	public void viExp(int num) throws Exception {
		...
		textRecordExp_clear("./result/viExp.txt");
		fhb.creTab("tempM1_guoRes");
		...
		for (int i = 0; i < num; ++i) {
			...
			// ....for cluster..//
			kviVal.conve_kvi_rangeSearchConsec(inte[0], inte[1]);
			...
			startCnt();
			qualnum = mrscanner.jobValRange(inte[0], inte[1], "tempM1_guo");
			stat[tnum][0] = stopCnt();
			stat[tnum++][1] = 100;
			...
			// for no-gridding process
			kviVal.kvi_rangeSearchNoGrid(inte[0], inte[1]);
			kviVal.kvi_metricOutput(tmpstat, tuprange);
			stat[tnum++][0] = tmpstat[0]; // tdur
			// stat[tnum][1] = tmpstat[1];// inmem
			// stat[tnum][2] = tmpstat[2];// randacc
			...
			textRecordExp("./result/viExp.txt", stat, tuprange, tnum, inte[0],
					inte[1]);
		}
	...
	public void vpExp(int num) throws Exception {
		...
		textRecordExp_clear("./result/vpExp.txt");
		fhb.creTab("tempM1_guoRes");
		String[] qual = { "vl", "vr" };
		...
		for (int i = 0; i < num; ++i) {
			...
			startCnt();
			qualnum = mrscanner.jobValPoint(inte[0], "tempM1_guo");
			stat[tnum][0] = stopCnt();
			stat[tnum++][1] = 100;
			...
			textRecordExp("./result/vpExp.txt", stat, tuprange, tnum, inte[0]);
		}
		return;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		} else if (sign.equals("l2") == true)// raw data
		{
			...
			QueryManager qrypro = new QueryManager(false);
			qrypro.ImportFolder("/home/guo/data/accel_data_yan/userAdd/set1",
					"gsensor", 0);

...
//
	...
	//
	// 1.272754426E9, 1.274610298E9, 1.269541917E9, 1.271397789E9,
	// 1.294482299E9,
	...
	public void qryGen_tempRange(double inter[], double tsel) {// inter[0]:left

		double inteRang = (edtime - sttime);
		double gap = (edtime - sttime) * tsel;
		long leftbd = 0, rightbd = 0;
		leftbd = (long) (sttime + Math.random() * inteRang);
		rightbd = (long) (leftbd + gap);
		if (rightbd > edtime) {
	...
	public void qryGen_valRange(double inter[],double sel) {// inter[0]:left

		// double inteRang = maxval - minval;

		double inteRang = (maxval - minval);
		double gap = (maxval - minval) * sel;
...
//			str += (Double.toString(timetup[numway - 1][1]) + ",");
	...
	public void viExp(int num) throws Exception {
		...
		for (int i = 0; i < num; ++i) {
			...
			//
			 startCnt();
			 qualnum = mrscanner.jobValRange(inte[0], inte[1],
			 "valIdxmodCol");
			 stat[tnum][0] = stopCnt();
			 stat[tnum++][1] = 100;
			//
			...
			textRecordExpKVIMRK("./result/vitest.txt", stat, tuprange,
					tnum, inte[0], inte[1]);
		}
	...
	public void newtiExp(int num) throws Exception {
		...
		for (int i = 0; i < num; ++i) {
			...
			textRecordExp("./dasfaa/test.txt", stat, tuprange, tnum,
					inte[0], inte[1]);
		}
...
//			System.out.printf("MRK %f  \n", kviVal.kvi_timeCnt());
	...
	public void newtiviExp(int num) throws Exception {
		...
		for (int i = 0; i < num; ++i) {
			...
			textRecordExp("./dasfaa/tivi.txt", stat, tuprange, tnum,
					intev[0], intev[1]);
...
//			System.out.printf("%f  \n", nkviVal.kvi_timeCnt());
			...
			textRecordExp("/dasfaa2/cqry1.txt", stat, tuprange, tnum,
					intev[0], intev[1]);
